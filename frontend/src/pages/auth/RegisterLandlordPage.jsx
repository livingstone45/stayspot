  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem('user', JSON.stringify(data.data?.user || { role: 'landlord' }));
        setLoading(false);
        const dashboardPath = getRoleDashboardPath(data.data?.user || { role: 'landlord' });
    ...
    <div style={{ 
      ...
      <div className="register-card" style={{ 
        ...
        <Link to="/" style={{ fontSize: '14px', fontWeight: '600', color: COLOR, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '16px' }}>
          <div style={{ width: '20px', height: '20px', background: `linear-gradient(135deg, ${COLOR} 0%, #ea580c 100%)`, borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: '700', fontSize: '9px' }}>
            SS
          </div>
          <span>StaySpot</span>
        </Link>
        ...
        <p style={{ color: '#6b7280', fontSize: '11px', marginBottom: '12px' }}>
          ...
          {step === 4 && 'Tell us about your properties'}
        </p>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          {step === 4 && (
            <>
              <div>
                <label style={{ display: 'block', marginBottom: '3px', fontWeight: '500', color: '#1f2937', fontSize: '11px' }}>Number of Properties Owned</label>
                <input 
                  type="number" 
                  name="propertyCount" 
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '3px', fontWeight: '500', color: '#1f2937', fontSize: '11px' }}>Years of Experience in Property Management</label>
                <input 
                  type="number" 
